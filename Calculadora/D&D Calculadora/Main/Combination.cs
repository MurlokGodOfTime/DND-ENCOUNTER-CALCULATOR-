...
class Combination
{
    ...
    internal void Main(int calcAnsw, int calcCap, int quant)
    {
        ...
        for (int i = 0; crX[i] < calcCap; i++)
        {
            limite = i;
        }
        List<List<int>> combinations = FindCombinationsInRange(crX, calcAnsw, calcCap, quant, limite);
    ...
    static List<List<int>> FindCombinationsInRange(int[] crX, int minSum, int maxSum, int combinationLength, int limite)
    {
        ...
        FindCombinationsInRangeHelper(crX, minSum, maxSum, combinationLength, 0, currentCombination, result, limite);

        return result;
    }

    static void FindCombinationsInRangeHelper(int[] crX, int minSum, int maxSum, int k, int start,
                                              List<int> currentCombination, List<List<int>> result, int limite)
    {
        if (k == 0)
        {
        ...
        for (int i = start; i < limite; i++)
        {
            ...
            FindCombinationsInRangeHelper(crX, minSum, maxSum, k - 1, i, currentCombination, result, limite); // i - 1 é melhor ou pior?
